...
class AzureProvider(BaseProvider):
    def get_models(self, model_id=None):
        ...
        if response.status_code == 200:
            ...
            raise Exception('Failed to get deployments from Azure OpenAI. Status code: {}'.format(
                response.status_code))

    def get_credentials(self, model_id=None):
    ...
    def get_provider_configs(self, obfuscated=False) -> Union[dict | str]:
        try:
        ...
        if obfuscated:
            ...
            config['openai_api_key'] = self.obfuscated_token(
    ...
    def get_encrypted_token(self, config):
        return json.dumps({
            'openai_api_type': 'azure',
            'openai_api_version': '2023-03-15-preview',
